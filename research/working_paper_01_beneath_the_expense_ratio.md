...
I am not arguing that those degradations are pervasive. For most funds, most of the time, they are not. The argument is that the data required to identify the funds where they are pervasive is already being filed under SEC rule. The aggregation is what is missing.
...
The substantive issue is that the adviser consumes the research, not the fund, while the fund pays for it through elevated commissions. The adviser would otherwise pay for the same research out of its management fee, so the arrangement shifts cost from the adviser's income statement to the fund's trading book, where it is disclosed as a commission rather than as a fee, and where it does not appear in the expense ratio. That is the mechanism by which a cost that is economically an adviser expense becomes an invisible cost to the fund shareholder.
...
The cost of drawn facility balances is interest expense, and the cost does appear in the expense ratio in the "other expenses" line if it was incurred during the period. But the signal value of utilization is about liquidity-stress detection, not cost. A passive index fund that tapped its credit line at 40% of facility size during a particular year was experiencing redemption pressure, market-dislocation-forced selling, or both. That history is informative about future behavior in similar stress scenarios.
...
The proposed weights reflect my judgment on relative economic magnitude, and they are not a claim about optimal weighting. The weights are published, they are versioned, and the underlying component scores are visible to any user of the tool.
